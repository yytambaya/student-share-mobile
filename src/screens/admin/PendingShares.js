...
const PendingShares = ({}) => {
  ...
  const [rejBtnState, setRejBtnState] = useState('Approve')
  ...
  const renderItem = ({ item }) => (
    <Box key={item._id} borderBottomWidth={1} borderColor="coolGray.200" p={4} mb={2} bg="coolGray.50" borderRadius="md">
      ...
      return <Box>
        ...
        <Menu.Item onPress={() => {setIsRejOpen(true); setID(item._id)}}>approve</Menu.Item>
  ...
  const RejConfirmation = () => {
    ...
    return <Center>
        ...
        <AlertDialog leastDestructiveRef={rejCancelRef} isOpen={isRejOpen} onClose={onCloseRej}>
          <AlertDialog.Content>
            <AlertDialog.CloseButton />
            <AlertDialog.Header>Approve Resource</AlertDialog.Header>
            <AlertDialog.Body>
            
            {error.find(item => item.field == "id").msg ? <Text color='red.500'>{error.find(item => item.field == "id").msg }</Text>: null}
              This content will be approved remove this post. The content of this post will be visible to
              student to view and download
            </AlertDialog.Body>
            <AlertDialog.Footer>
              <Button.Group space={2}>
                ...
                <Button colorScheme="danger" onPress={() => approveShare()}>
                  {rejBtnState}
                </Button>
              </Button.Group>
            </AlertDialog.Footer>
