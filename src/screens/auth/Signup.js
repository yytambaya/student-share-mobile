...
const Signup = () => {
    ...
    const [error, setError] = useState([{field: "name", msg:""}, {field: "email", msg:""}, {field: "regNo", msg:""}, {field: "password1", msg:""}, {field: "password2", msg:""}]);
    ...
    const signup = async () => {
        ...
        var ph_val = validateName(regNo).error == "" ? true: false;
        ...
        if(n_val && e_val && ph_val && p1_val && p2_val){
            console.log("fields are okay")
            if(password1 == password2){
              console.log("passwords are equal")
            setBtnState("Loading...")
            const url = `${getAPIBaseURL()}/v1/user/signup`;
            const api_key = '@!8(T#7<R:I#:F1#r!>BW/!'
            const headers = {'x-access-key': api_key}
            const data = {name: name, email: email, regNo: regNo, password: password1, status: 1};
